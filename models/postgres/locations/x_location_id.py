...
class XLocationID(db.Model):
    """
    Table that holds information about X Location ID.
    """
    __bind_key__ = "postgres"

    __tablename__ = "x_location_ids"
    ...
    def __table_args__(self):
        return (
            db.Index(
                'idx_x_location_ids_region_id',
                self.region_id,
                postgresql_using='hash'
            ),
            db.Index(
                'idx_x_location_ids_subregion_id',
                self.subregion_id,
                postgresql_using='hash'
            ),
            db.Index(
                'idx_x_location_ids_country_id',
                self.country_id,
                postgresql_using='hash'
            ),
            db.Index(
                'idx_x_location_ids_state_id',
                self.state_id,
                postgresql_using='hash'
            ),
            db.Index(
                'idx_x_location_ids_city_id',
                self.city_id,
                postgresql_using='hash'
            ),
            db.Index(
                'idx_x_location_ids_vector',
                self.search_vector,
                postgresql_using='gin'
            ),
            db.Index(
                'idx_x_location_ids_trgm',
                self.name,
                postgresql_using='gin',
                postgresql_ops={
                    'name': 'gin_trgm_ops'
            ...
            # db.Index(
            #     'idx_x_location_ids_geom',
            #     self.geom,
            #     postgresql_using='gist',
            # ),
            db.Index(
                'idx_x_location_ids_ll_to_cube',
                self.ll_to_cube,
                postgresql_using='gist',
            ),
        )
...
@db.event.listens_for(XLocationID, 'before_insert')
@db.event.listens_for(XLocationID, 'before_update')
def update_x_location_search_vector(_, __, target):
...
@db.event.listens_for(XLocationID, 'before_insert')
@db.event.listens_for(XLocationID, 'before_update')
def update_x_location_id_null_columns(_, __, target):
    """
