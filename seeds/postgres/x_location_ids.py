...
with app.app_context():
    metadata = MetaData()
    table_x_locations = db.Table(
        XLocationID.__tablename__,
        metadata=metadata,
        bind_key="postgres",
        autoload_with=engine)

    if table_exists(db, table_x_locations.name):
        ...
        for fk_name in fk_names:
            if fk_name:
                sql_text = (f'ALTER TABLE {table_x_locations.name} '
                            'DROP FOREIGN KEY '
                            f'{fk_name}')
                db.session.execute(text(sql_text))

    table_x_locations.drop(engine, checkfirst=True)
    table_x_locations.create(engine, checkfirst=True)
    try:
        # db.session.add_all([
        #     XLocationID()
        # ])
        db.session.commit()
        print(XLocationID.__tablename__ + " seeded successfully!")
