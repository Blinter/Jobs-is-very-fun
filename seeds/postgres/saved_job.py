...
with app.app_context():
    ...
    table_saved_job = db.Table(
        SavedJob.__tablename__,
        metadata=metadata,
        bind_key="postgres",
        autoload_with=engine)

    if table_exists(db, table_saved_job.name):
        ...
        for fk_name in fk_names:
            if fk_name:
                sql_text = (
                    f'ALTER TABLE {table_saved_job.name} \
DROP FOREIGN KEY {fk_name}')
                db.session.execute(text(sql_text))

    table_saved_job.drop(engine, checkfirst=True)
    ...
    table_saved_job.create(engine, checkfirst=True)

    try:
        # test_saved_job = SavedJob()
        # db.session.add_all([
        #     test_saved_Job
        # ])
        db.session.commit()
        print(SavedJob.__tablename__ + " seeded successfully!")
